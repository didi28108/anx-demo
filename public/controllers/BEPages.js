			...
			if($scope.currentCategory) {
				if($scope.panelVisible) {
					if($window.confirm("是否放棄更動當前頁面？")){
						...
						document.getElementById('pagePanel').classList.remove('panel-warning');
						document.getElementById('pagePanel').classList.add('panel-info');
		...
		$scope.prepareEditPage = function () {
			...
			if(!$scope.panelVisible) {
				...
				PagesService.getPage(this.page.abbr).then(function (data) {
					...
					document.getElementById('pagePanel').classList.remove('panel-info');
					document.getElementById('pagePanel').classList.add('panel-warning');
